...
class Auth {
	...
	async register(params: any): Promise<void | any> {
		const data = await this.#host
	...
	async login(params: { email: any; password: any; reload: any; }): Promise<void | any> {
	...
	async changePassword(params: { email: string; old_password: string; new_password: string }) {
